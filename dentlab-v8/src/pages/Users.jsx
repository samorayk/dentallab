...
export default function UsersPage() {
  ...
  // ── Password change using Supabase Admin REST API directly ──
  const changePassword = async () => {
    ...
    if (!serviceKey || !url) {
      // No service key — show password to communicate manually
      setPwdInfo(`📋 Mot de passe à communiquer manuellement:\n\n🔑 ${pwdForm.newPwd}\n\n⚠ Pour activer le changement automatique, ajoutez VITE_SUPABASE_SERVICE_ROLE_KEY dans votre .env`);
      return;
    }
          ...
          <div key={p.id} style={{ padding: '12px 14px', borderBottom: '1px solid ' + c.bdrL, display: 'flex', gap: 10, alignItems: 'center' }}>
            ...
            <div style={{ flex: 1, minWidth: 0 }}>
              ...
              <div style={{ fontSize: FS - 3, color: c.txL }}>
                {t(p.role)} {p.phone ? '· ' + p.phone : ''} {p.clinic ? '· ' + p.clinic : ''}
              </div>
            </div>
            <div style={{ display: 'flex', gap: 6 }}>
              <Btn sm onClick={() => { setInfo(''); setForm({ ...p, email: '' }); }}>✏️</Btn>
              <Btn sm onClick={() => { setPwdInfo(''); setPwdForm({ userId: p.id, name: p.name, newPwd: '', confirmPwd: '' }); }}
                style={{ background: '#FEF3C7', color: '#92400E', border: '1px solid #FDE68A' }}>🔑 MDP</Btn>
      ...
      {form && (
        <Modal onClose={() => { setForm(null); setInfo(''); }} w={520}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8 }}>
            ...
            <div>
              <Lbl>Email *</Lbl>
              <Inp type="email" value={form.email || ''} disabled={!!form.id}
                onChange={e => setForm({ ...form, email: e.target.value })}
                style={{ opacity: form.id ? 0.5 : 1 }} />
            </div>
            <div>
              <Lbl>Rôle</Lbl>
        ...
        <Modal onClose={() => { setPwdForm(null); setPwdInfo(''); }} w={420}>
            ...
            <div>
              ...
              <div style={{ position: 'relative' }}>
                ...
                <button type="button" onClick={() => setPwdVisible(v => !v)}
                  style={{ position: 'absolute', right: 10, top: '50%', transform: 'translateY(-50%)', background: 'none', border: 'none', cursor: 'pointer', color: c.txL, fontSize: FS }}>
                  {pwdVisible ? '🙈' : '👁'}
                </button>
              </div>
            </div>
          ...
          {pwdInfo && (
            <div style={{ marginTop: 12, padding: 12,
              background: pwdInfo.startsWith('✓') ? '#D1FAE5' : pwdInfo.startsWith('💡') ? '#FEF3C7' : pwdInfo.startsWith('📋') ? '#EFF6FF' : '#FEF2F2',
              color: pwdInfo.startsWith('✓') ? '#065F46' : pwdInfo.startsWith('💡') ? '#92400E' : pwdInfo.startsWith('📋') ? '#1E40AF' : c.dng,
